# Méthode getrawtransaction

Retourne les informations de la transaction correspondante basé sur la valeur de hashage spécifiée

## Paramètres de la méthode

txid : ID de la transaction.

verbose : Optionnel. Par défaut, la valeur de "verbose" est 0. Quand "verbose" est à 0, l'information sérialisé de la transaction est retourné représenté par un string hexadécimal. Si vous avez besoin d'informations plus détaillées, vous pouvez utiliser le SDK pour la désérialisation. Quand il est à 1, les informations détaillées de la transaction correspondant sont données au format JSON.
