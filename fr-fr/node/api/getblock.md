# Méthode getblock

Retourne les informations du bloc correspondant à la valeur de hashage indiquée.

## Paramètres de la méthode

hash : La valeur de hashage du bloc.

verbose : Optionnel. Par défaut, la valeur de "verbose" est 0. Quand "verbose" est à 0, l'information sérialisé du bloc est retourné représenté par un string hexadécimal. Si vous avez besoin d'informations plus détaillées, vous pouvez utiliser le SDK pour la désérialisation. Quand il est à 1, les informations détaillées du bloc correspondant sont données au format JSON.

## Exemples

Requète :

```json
{
  "jsonrpc": "2.0",
  "method": "getblock",
  "params": ["773dd2dae4a9c9275290f89b56e67d7363ea4826dfd4fc13cc01cf73a44b0d0e"],
  "id": 1
}
```

Réponse :

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": "  "
}
```

Requète :

verbose = 1.

```json
{
  "jsonrpc": "2.0",
  "method": "getblock",
  "params": ["773dd2dae4a9c9275290f89b56e67d7363ea4826dfd4fc13cc01cf73a44b0d0e", 1],
  "id": 1
}
```

Réponse :

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": {
    "Hash": "773dd2dae4a9c9275290f89b56e67d7363ea4826dfd4fc13cc01cf73a44b0d0e",
    "Size": 686,
    "Version": 0,
    "Previousblockhash": "282293a89587d2513ae82d5baf69a4afd68b8d2ac70e80f58246bc2ca8dfea2d",
    "Merkleroot": "2033d1779cef38259dc9df82a4deb258a944f807f820a4cc364105f11b08f816",
    "Time": 1496721145,
    "Index": 991956,
    "Nonce": "2a551c84bd408f87",
    "Nextconsensus": "APyEx5f4Zm4oCHwFWiSTaph1fPBxZacYVR",
    "Script": {
      "Invocation": "",
      "Verification": ""
    },
    "Tx": [
      {
        "Txid": "2033d1779cef38259dc9df82a4deb258a944f807f820a4cc364105f11b08f816",
        "Size": 10,
        "Type": "MinerTransaction",
        "Version": 0,
        "Attributes":[],
        "Vin":[],
        "Vout":[],
        "Sys_fee": "0",
        "Net_fee": "0",
        "Scripts":[],
        "Nonce": 3175124871
       }
    ],
    "Confirmations": 20,
    "Nextblockhash": "0b08e2eeed05c70f27293521c47f7f60dfc29f9f299ae9909a8552a4a87db7a2"
  }
}
```