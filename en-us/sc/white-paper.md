...
At present, the NEO community has been the emergence of some use of NEOs intelligent contract to achieve the blockchain superconducting transactions, such as blue whale scouring.
...
At present, the smart contract on the system can only access the data in its own storage area, the future may need a shared public storage between the contract, the storage area through the hash table to retrieve the file, and distributed Stored on the nodes of the small NEOs. May be implemented through DHT (Distributed Hash Table) or IPFS (InterPlanetary File System).
