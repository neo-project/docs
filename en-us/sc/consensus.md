...
* **Practical Byzantine Fault Tolerance** `PBFT` - A cosensus algorithm implemented within the AntShares block chain to guarantee fault tolerance.
...
1. **Consensus Node** - This node participates in the consensus activity.  During a consensus activity, consensus nodes take turns assuming the following two roles:
    ...
    - **Congressman** `(Many)` - Congressman nodes are analogous to lieutenants in section 4. They are responsible for reaching a consensus on a transaction.
  
2. **Bookkeeper Node** `(Many)` -  The bookkeeper nodes act as scribes and digitally sign the blocks that are generated by consensus nodes. They must agree with the result of the consensus activity to sign the block.
...
5. At least `s` **Consensus Nodes** are in same state (`h`,`k`) to begin a consensus activity
