...
> [!Note]
>
> This document provides information based on [Neo CLI v2.10.0](https://github.com/neo-project/neo-cli/releases/tag/v2.10.0). 
...
> [!Note]
>
> Two new RPC API,  `getunclaimedgas` and`claimgas`, are added to NEO-CLI 2.10.0 and later versions for easier remote invoking. 

#### RPC method: getunclaimedgas

This method is used to get the unclaimed GAS amount in current wallet. 

To invoke this method, you can send the JSON request file with following text to RPC server.
...
This method claims the GAS and send them to the first standard address in the wallet by default. You can also specify an address to receive these GAS.

"params"  includes at lease one array with one parameter.   
...
```
{
  "jsonrpc": "2.0",
  "method": "claimgas",
  "params": ["AH2TGXkKgiWm4xMQzVT5R9zV1yxwVXNAPT"],
  "id": 1
}
```
