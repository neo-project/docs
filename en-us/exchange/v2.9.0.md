...
- As a wallet, manages assets through the command-line.
  ...
  - Before running NEO-CLI, you need to configure the config.json file to enable the client to invoke APIs related to a wallet. The following is an example:  
...
4. Send transaction to the user's withdrawal address using the NEO-CLI API method, `sendtoaddress <asset_id> <address> <value>`. For more information, refer to  [sendtoaddress Method](../node/cli/2.9.0/api/sendtoaddress.md).
...
The following shows an example of the API invoking request.
