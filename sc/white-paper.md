...
开发者可以直接使用几乎任何他们擅长的高级语言来进行小蚁智能合约的开发工作。第一批被支持的语言是 C#、VB.Net、F#等，小蚁提供了这些语言的编译器和插件，用于将高级语言编译成小蚁虚拟机所支持的指令集。由于编译器会针对 MSIL（微软中间语言）来进行编译，所以理论上任何.Net 中的语言或者可被转译成 MSIL 的语言都可以直接被直接支持。 
