...
トランザクションは、拡張用途のためのデータを含む場合があり、これらのデータはtransaction attributesフィールドに格納されます。
...
トランザクションにおいて、コントラクトスクリプトコードのハッシュ値はバリデーションの一部であるトラザクションOutputと一貫していなければなりません。後のセクションでは、スクリプトの実行プロセスについて詳しく説明します。
