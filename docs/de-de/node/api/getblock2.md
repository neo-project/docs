# Getblock Methode

Holt die zu einem bestimmten Index gehörenden Blockinformationen.

## Parameter Beschreibung

Index: Block Index (Blockhöhe) = Gesamtmenge der Blocks - 1.

Verbose: Optional ist der Standardwert von verbose 0. Wenn verbose 0 ist, wird die serialisierte Information des Blocks zurückgegeben, die durch eine hexadezimale Zeichenfolge dargestellt wird. Wenn Sie detaillierte Informationen benötigen, müssen Sie das SDK zur Deserialisierung verwenden. Wenn verbose 1 ist, werden detaillierte Informationen des entsprechenden Blocks im Json-Formatstring zurückgegeben.

## Beispiel

Anfrage:

```json
{
  "jsonrpc": "2.0",
  "method": "getblock",
  "params": [10000],
  "id": 1
}
```

Antwort:

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": "  "
}
```

Anfrage:

Verbose = 1, gibt das Ergebnis im JSON-Format aus.

```json
{
  "jsonrpc": "2.0",
  "method": "getblock",
  "params": [10000, 1],
  "id": 1
}
```

Antwort:

```json
{
  "jsonrpc": "2.0",
  "id": 1,
  "result": {
    "Hash": "4c1e879872344349067c3b1a30781eeb4f9040d3795db7922f513f6f9660b9b2",
    "Size": 686,
    "Version": 0,
    "Previousblockhash": "53ce15a53a184faa058be9008ece887d7f0b30a459342ce42753f73f9ed390e9",
    "Merkleroot": "9c909e1e3ba03290553a68d862e002c7a21ba302e043fc492fe069bf6a134d29",
    "Time": 1476834896,
    "Index": 10000,
    "Nonce": "aa2c0fd5dc445445",
    "Nextconsensus": "APyEx5f4Zm4oCHwFWiSTaph1fPBxZacYVR",
    "Script": {
      "Invocation": "",
      "Verification": ""
    },
    "Tx": [
      {
        "Txid": "9c909e1e3ba03290553a68d862e002c7a21ba302e043fc492fe069bf6a134d29",
        "Size": 10,
        "Type": "MinerTransaction",
        "Version": 0,
        "Attributes":[],
        "Vin":[],
        "Vout":[],
        "Sys_fee": "0",
        "Net_fee": "0",
        "Scripts":[],
        "Nonce": 3695465541
      }
   ],
    "Confirmations": 981987,
    "Nextblockhash": "c9a32cd215837c02dd72613bfad6ed064579e77bafd859907f87b3e67fa498cc"
  }
}
```