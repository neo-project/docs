...
1. 构建地址脚本合约 
   ...
   (脚本合约格式：`0x21`(1字节,代表Opcode中PushBytes指令)+压缩型公钥(33字节) + `0xac`（1字节,代表Opcode中 CheckSig指令))
...
6. 取data[7-38]作为加密密钥Encryptedkey（32字节），并用导出半数2 Derivedhalf2作为初始向量对其进行AES256解密
