...
`Neo RPC SDK` 封装了交易构造模块，通过该模块可以使用特定的参数和方法构造 Neo N3 中的交易，完成个性化的功能，本篇主要介绍这部分的使用方法。
...
2. 构造 `TransactionManagerFactory`，将`RpcClient`和 `Magic`作为参数; 构造`TransactionManager`, 将`Script`和`Signers`作为参数。
