...
需要注意的是，在 Neo3 中手续费是可以动态变化的，下面列举的是基础费用。委员会可以决定在基础费用上乘上一个倍率变为最终费用。
...
首先要计算验证脚本的正常执行费用，即 操作码执行费用+系统调用费用+原生合约 CPU 使用费用+存储区使用费用，然后乘以 `验证脚本默认执行费率倍率`，且最大不得超过 0.5 GAS。
