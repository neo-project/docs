...
NEO 的共识节点由 NEO 持有者投票选出。详细信息可参考 http://neofans.org/2018/11/02/how-to-become-a-consensus-node/
...
使用区块链浏览器（如 https://neoscan.io/）查看最近区块高度，然后查看您客户端中的钱包高度，如果高度一样，说明客户端已完全同步。
...
可以制作一些脚本，比如在 Windows 上创建一个记事本文件，输入`dotnet neo-cli.dll /rpc` 然后保存为.cmd文件或者在 linux 上使用screen命令。
...
NEO 支持多种主流编程语言，可以使用 C#、Python、Go 和 Jave 进行开发。目前在NEO生态中，C# 与 Python具有非常完善的基础设施，提供多种编译器可供开发者选择。
