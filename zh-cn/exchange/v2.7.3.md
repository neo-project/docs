...
- 用户第一次充值（NEO/NEO GAS）时，程序动态创建 NEO 地址，优点：无需人工定期创建地址；缺点：不方便备份钱包。
  ...
  要动态创建地址，可以使用 NEO-CLI API 的 [getnewaddress方法](../node/cli/2.7.3/api/getnewaddress.md) 实现。程序会返回创建的地址。
...
1. 使用NEO-CLI  API （[getblock方法](../node/cli/2.7.3/api/getblock2.md)）监控新区块。
2. 根据交易信息完成用户充值。
3. 存储交易所相关交易记录。
...
NEO-CLI  API 中的 getblock <index> [verbose] 方法提供了获取区块信息的功能，该方法中的 <index> 为区块索引。[verbose] 默认值为 0，表示返回的是区块序列化后的信息，用 16 进制字符串表示，如果从中获取详细信息需要反序列化。[verbose] 为 1 时返回的是对应区块的详细信息，用 Json 格式字符串表示。更多信息请参阅 [getblock方法](../node/cli/2.7.3/api/getblock2.md)。
...
4. 使用NEO-CLI API 中的 `sendtoaddress <asset_id> <address> <value>` 方法 ，向用户提现地址发送交易。更多信息，请参阅 [sendtoaddress方法](../node/cli/2.7.3/api/sendtoaddress.md)。

   - `<asset_id>` ：资产ID
   - `<address>` ：提现地址
   - `<value>` ：提现金额

   要向多个地址批量发送交易，可以使用 API [sendmany方法](../node/cli/2.7.3/api/sendmany.md)。
...
要使用 [getapplicationlog](../node/cli/2.7.3/api/getapplicationlog.md) 这个api来获取交易信息，必须运行以下命令来打开 neo-cli 客户端：
