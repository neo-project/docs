...
> [!Note]
>
> -  在 neo-cli-2.10.0及以后版本中，添加了 RPC `getunclaimedgas`, `claimgas` 两个新的 api, 方便进行远程调用。

#### RPC 方法：getunclaimedgas

"params"  包含一个至少一个参数的数组。

`"params":[]`

打开钱包后，想要知道目前钱包内所有地址生成的GAS情况，可以编写如下 JSON 文件并发送给 RPC 服务器。
...
默认将 GAS 提取到钱包第一个标准地址。也可以指定地址作为参数，将 GAS 提取到指定地址。可以编写如下 JSON 文件并发送给 RPC 服务器。
