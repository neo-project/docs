...
> [!Note]
>
> 本文对应neo-cli版本为 [Neo CLI v2.7.6](https://github.com/neo-project/neo-cli/releases/tag/v2.7.6)
...
要使用 [getapplicationlog](../node/cli/latest-version/api/getapplicationlog.md) 这个api来获取交易信息，必须运行以下命令来打开 neo-cli 客户端：
...
> [!Note]
>
> -  必须注意的是一旦开启 --rpc， 就会从当前区块高度开始获取对应区块的log。 如果从0开始获取，log文件将是海量的，会大量占用 linux 系统下的 inode 数， 甚至可能导致 no space left on device 的 exception。 对于交易所来说如果列入了 nep-5 资产， 只需从该资产的发行区块高度开始捕获 log 即可。如果只列入UTXO， 那只需根据具体所需从相应的区块高度开始捕获Log。
> -  强烈建议定期删除无用的log，比如 30 天前的 log， 以避免 inode 爆满。

可以看到在根目录下生成了一个 ApplicationLogs 文件夹，完整的合约日志会记录到该目录下，每笔 NEP-5 交易会记录在一个 JSON 文件中，文件名为该交易的 txid。
