...
1. 使用 neo-cli  API （[getblock 方法](../node/cli/latest-version/api/getblock2.md)）监控新区块。
2. 根据交易信息完成用户充值。
3. 存储交易所相关交易记录。
...
4. 使用 neo-cli API 中的 `sendtoaddress <asset_id> <address> <value>` 方法 ，向用户提现地址发送交易。更多信息，请参阅 [sendtoaddress 方法](../node/cli/latest-version/api/sendtoaddress.md)。
...
使用 [getapplicationlog](../node/cli/latest-version/api/getapplicationlog.md) 这个 api 来获取交易信息。
