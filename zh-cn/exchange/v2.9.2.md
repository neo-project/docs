...
| [ImportBlocks](https://github.com/neo-project/neo-plugins/releases/download/v2.9.2/ImportBlocks.zip) | 同步离线包，发布的版本中已经自带。                           | 交易所必选   |
...
1. 使用 NEO-CLI  API （[getblock 方法](../node/cli/2.9.2/api/getblock2.md)）监控新区块。
2. 根据交易信息完成用户充值。
3. 存储交易所相关交易记录。
