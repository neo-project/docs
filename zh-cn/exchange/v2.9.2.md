...
3. 从 GitHub 上下载 [Plugins](https://github.com/neo-project/neo-plugins/releases) 中的 [ApplicationLogs](https://github.com/neo-project/neo-plugins/releases/download/v2.9.2/ApplicationLogs.zip) 和 [ImportBlocks](https://github.com/neo-project/neo-plugins/releases/download/v2.9.0/ImportBlocks.zip) 以保证交易日志 API 和自动读取离线包的完整性。
...
5. 在启动 neo-cli 前需先配置 config.json 文件，开启自动绑定并打开钱包功能。钱包打开后才可以调用与钱包相关的 api。下面是一个标准设置的例子，其中 Path 是钱包的路径， Password 是钱包的密码， IsActive 设为 true 表示允许自动打开钱包。
...
> [!Note]
>
> 在 NEO-CLI 2.9.1中新增了 BindAddress 选项，默认为本地127.0.0.1。可以绑定指定网卡的 ipv4 地址以允许远程调用 rpc。若没有指定对象，则可以设成 0.0.0.0。
