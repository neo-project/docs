...
1. 进入 [客户端下载](https://neo.org/download) 页面，根据您所在的网络选择下载主网数据包（chain.acc.zip）或测试网数据包（chain.acc.test.zip）
   ...
   如果是测试网数据包，将文件名改为chain.acc.zip。如果是主网数据包则无需更改。

2. 关闭客户端。将 chain.acc.zip 复制到 neo-cli.exe 文件夹中。

3. 运行 `dotnet neo-cli.dll` 打开 NEO 命令行钱包。

4. 输入 `show state` 命令查看区块同步状态，"Height"后面的三个数值相同表示同步完成。
