...
以m个公钥创建多方签名的合约地址，例如两个公钥创建的多方签名地址， m可以为1或2， 后面的参数就是两方的公钥。

👉 `sign <jsonObjectToSign>` 

从签名数量为1以上的多方签名中合约中提取资产时，需要多方进行签名，参数为记录这段交易的json字符串。 签名完整后交易才能成功。

👉 `relay <jsonObjectToSign>` 

签名完整并且交易成功后，可以将这段交易信息进行广播。参数同样为记录这段交易的json字符串。
