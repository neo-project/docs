...
| claim gas [all]                          | 提取钱包中的前50个可提取 GAS 的地址的 / 所有可提取的 GAS             | 需要打开钱包 |
...
提取钱包中的所有可提取的 GAS，该过程是通过一个特殊的交易 Claim Transaction 完成的，输入命令后，客户端会输出 Claim Transaction 的 ID（提取 GAS 这笔交易的 ID）。用户可以有两种选择，如末尾不带参数，则默认claim 钱包前50个可提取 GAS 的地址中的 GAS。如果用户加上参数[all]，则会提取钱包中所有地址的 GAS。
