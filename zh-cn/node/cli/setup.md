...
3. 在启动 neo-cli 前需先配置 config.json 文件，以允许调用钱包相关 api 以及开启自动绑定并打开钱包的功能。下面是一个标准设置的例子。
...
> 钱包打开后才可以调用与钱包相关的 api，config.json 提供了自动开启钱包的选项，不过不是必须项。Path 是钱包的路径， Password 是钱包的密码， IsActive 设为 true 意味着允许自动打开钱包。
