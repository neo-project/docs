...
由于区块链数据庞大，初次同步时等待时间通常很久，建议下载[离线同步包](https://www.neo.org/client/chain.acc.zip)，下载后的数据应该是一个chain.acc.zip的压缩包，将其解压后是一个chain.acc的文件。可以将chain.acc文件或chain.acc.zip文件复制到neo-gui.exe所在目录中，然后打开客户端，会发现客户端以超快的速度进行同步。如果之前已经同步过，neo-gui.exe所在目录会有一个Chian文件夹，将其改名或移除后，再把离线同步包放入并启动客户端，离线同步包才会有效。
...
钱包用来存储 NEO 账户及账户中资产信息，是客户端的钱包数据库文件，以 .json 或.db3 为后缀名称。该文件非常重要，需要**安全备份**。
