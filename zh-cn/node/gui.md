...
该账户所拥有的资产信息，资产信息包括：资产（NEO 股、NEO 币、用户所创设的资产）、类型、余额、发行者。
...
在 NEO 区块链中创设一种新的用户发行资产。用户可以自己定义资产的类型、名称、总量等，并指定资产的管理员账户。创设资产需要消耗一定数量的 NEO 币作为附加服务费，目前为 10000 NEO 币（测试网手续费为主网的百分之一）。

#### 分发资产

在资产创设所设定的总量上限范围内，向发行人指定的地址中发放该资产。分发资产需要消耗一定数量的 NEO 币作为附加服务费，目前为 500 NEO 币（测试网手续费为主网的百分之一）。

#### 选举

想要成为 NEO 记账人的用户，需通过选举进行登记。通过抵押一定量的 NEO 币，获得候选记账人资格。目前记账人功能暂未开放，请等候通知。

#### 投票

NEO 股持有者对候选记账人进行投票，根据投票结果，确定记账人。目前记账人功能暂未开放，请等候通知。
...
NEO 币伴随着每个新区块的生成而产生，会按照 NEO 股的持股比例，记录在 NEO 股地址上（资产余额中的括号里的数目即为待认领的 NEO 币数目）。NEO 股持有人可以在任意时间进行发起一笔认领交易，将这些 NEO 币认领到 NEO 股的地址上。目前只有 PC 版客户端拥有提取 NEO 币的功能。

具体操作步骤为：

(1) 对钱包中的所有 NEO 股进行一次转账操作（可以直接转到目前地址），待认领的 NEO 币才会结算一次，变得可提取（具体原理见白皮书）。

(2) 点击 `高级`，`提取 NEO 币`，`全部提取`，提取完成。
...
需要注意的是，注册资产需花费大量手续费（主网 10000 NEO 币，测试网 100 NEO 币），请谨慎操作。

### 分发资产

![](~/images/gui/A1.png)

需要注意的是，分发资产需花费大量手续费（主网 500 NEO 币，测试网 5 NEO 币），请谨慎操作，可以选择一次性分发完毕。
...
需要注意的是，选举需花费大量手续费（主网 1000 NEO 币，测试网 10 NEO 币），请谨慎操作。目前记账人权限暂未开放，请等候通知。
