...
由于区块链数据庞大，初次同步时等待时间通常很久，建议下载[离线同步包](https://www.neo.org/client/chain.acc.zip) 并将其复制到 Neo-GUI 程序文件夹内。添加离线同步包后打开客户端，将发现客户端以超快的速度进行同步。

![](assets/gui_1.png)

“高度”旁边的三个数据依次为：钱包高度/客户端高度/区块高度，当创建或者打开一个钱包后，钱包高度会显示钱包的同步数据。

### 术语解释

**钱包**

用来存储 NEO 账户及账户中资产信息的文件，也就是客户端的钱包数据库文件，以 .json 或.db3 为后缀名称。该文件非常重要，需要 **安全备份**。
...
4. 点击 `确定` 后，钱包创建成功，此时钱包里会默认带有一个标准账户。
   ...
   右键单击钱包可以创建更多地址。需要注意的是，因找零机制的作用，根据转账金额，转账后的剩余部分资产会默认转到第一个地址里，所以需备份好相应的私钥与钱包。
...
1. 在客户端中点击 `交易` ->`转账` -> `+` ，输入转账信息，如资产类型，转入地址和金额。
...
1. 点击 `交易` -> `转账` -> `+` ，输入转账信息，如资产类型，转入地址和金额。
